...
test("compressor throw => fallback raw", () => {
  // generic an toàn; test path lỗi giả lập qua input hợp lệ vẫn không throw -> kiểm tra registry không vỡ
  const input: CompressInput = { stdout: "x", stderr: "", exitCode: 0, command: "git diff" };
  const r = compress("git", input, defaultConfig);
