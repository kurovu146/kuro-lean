...
| `💵 wk $1.7k 2.7B` | What the current quota week has cost so far, across **every** project on this machine, priced with the machine-wide `pricing` table (defaults + `~/.claude/kt.json`, never one project's). The week is anchored to Claude Code's real `resets_at`, not a guessed weekday, and falls back to a rolling seven days when that reset has already passed. Refreshed in the background every 10 minutes and cached in your temp dir — the scan runs detached, never on the line: measured over 2,016 transcripts (1.12 GB), ~0.5–0.9s and ~290 MB peak RSS for the files inside the window. If every file looks fresh — after a backup restore, an `rsync`, or copying `~/.claude` to a new machine — nothing can be skipped and it costs ~3.4–4.1s and ~400–520 MB instead. A model with no entry in `pricing` still contributes its tokens, and the money reads `$1.7k+?` so the gap is visible. Turn it off with `"statusline": { "weekly": false }` |
